...
def get_num_classes(dataset_name: str) -> int:
    ...
    if dataset_name in ["mnist", "cifar10", "svhn"]:
        ...
        num_classes = 200
    elif dataset_name in ('flower102', 'flowers102'):
        num_classes = 102
...
def dataset_and_transform_generate(args):
    ...
    if (train_dataset_without_transform is None) or (test_dataset_without_transform is None):
        ...
        elif args.dataset in ("flower102", "flowers102"):
            from torchvision.datasets import Flowers102
            flowers_target_transform = transforms.Lambda(lambda label: label - 1)
            train_dataset_without_transform = Flowers102(
                args.dataset_path,
                split='train',
                transform=None,
                target_transform=flowers_target_transform,
                download=True,
            )
            test_dataset_without_transform = Flowers102(
                args.dataset_path,
                split='test',
                transform=None,
                target_transform=flowers_target_transform,
